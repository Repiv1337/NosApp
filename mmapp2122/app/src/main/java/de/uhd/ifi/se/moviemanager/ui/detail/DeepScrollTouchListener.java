...
public class DeepScrollTouchListener implements View.OnTouchListener {
    ...
    private boolean hasReachedVerticalLimit(final int direction) {
        ...
        return (textView
                .getScrollY() == 0 && direction == MOVE_DOWN) || (textView
                .getScrollY() == maxScroll && direction == MOVE_UP);
    }

    private int calculateMaxScroll() {
        int maxScroll = textView.getLineHeight() * (textView.getLayout()
                .getLineCount() - textView.getMaxLines()) + 2;
        if (maxScroll < 0)
            maxScroll = 0;
